...
public class ReviewController {
    ...
    //리뷰 게시글 전체 조회
    @GetMapping("/review")
    public ReviewListPageResponse findAllReview(Pageable pageable){
        return reviewService.findAll(pageable);
    }

    //리뷰 게시글 단건 조회
    @GetMapping("/review/{reviewId}")
    public ReviewDetailResponse findAllReview(@PathVariable Long reviewId){
        return reviewService.findById(reviewId);
    }

    //리뷰 게시글 수정
    @PatchMapping("/review/{reviewId}")
    public ReviewDetailResponse editReview(UserSession userSession, @PathVariable Long reviewId, @RequestBody @Valid ReviewEdit reviewEdit){
